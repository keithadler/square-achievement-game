...
/* ---- strategy extraction ---- */
static long long leaves=0, snodes=0; static int maxprint=6;
static int wins_now(BB me,int c);
static int bestmove(BB me,BB opp,int rem){
    BB occ=bor(me,opp), empty=bandn(FULL,occ);
    for(int i=0;i<W;i++){ u64 x=empty.w[i];
      while(x){ int k=__builtin_ctzll(x); x&=x-1; int c=(i<<6)+k;
        if(wins_now(me,c)) return c; } }
    for(int i=0;i<W;i++){ u64 x=empty.w[i];
      while(x){ int k=__builtin_ctzll(x); x&=x-1; int c=(i<<6)+k;
        BB nm=me; bset(&nm,c);
        if(-solve(opp,nm,-1,1,rem-1)==1) return c; } }
    return -1;
}
static void cellname(int c,char*b){ sprintf(b,"%c%d", 'a'+(c%N), N-(c/N)); }
static int wins_now(BB me,int c){ BB nm=me; bset(&nm,c);
    for(int j=0;j<ncellsq[c];j++){ BB m=sqmask[cellsq[c][j]]; if(bsub(&m,&nm)) return 1; } return 0; }
static void walk(BB me,BB opp,int rem,int ply,char*line){
    snodes++;
    int c=bestmove(me,opp,rem);
    if(c<0){ fprintf(stderr,"EXTRACTION FAILURE at ply %d\n",ply); exit(2); }
    char nm[8]; cellname(c,nm);
    char nl[512]; snprintf(nl,sizeof nl,"%s %s",line,nm);
    if(wins_now(me,c)){ leaves++; if(leaves<=maxprint) printf("  WIN:%s\n",nl); return; }
    BB m2=me; bset(&m2,c);
    BB occ=bor(m2,opp), empty=bandn(FULL,occ);
    for(int i=0;i<W;i++){ u64 x=empty.w[i];
      while(x){ int k=__builtin_ctzll(x); x&=x-1; int d=(i<<6)+k;
        BB o2=opp; bset(&o2,d);
        /* if Second wins immediately here the strategy is broken */
        if(wins_now(opp,d)){ fprintf(stderr,"SECOND WINS -- strategy invalid\n"); exit(3); }
        char dn[8]; cellname(d,dn);
        char nl2[512]; snprintf(nl2,sizeof nl2,"%s %s",nl,dn);
        walk(m2,o2,rem-2,ply+2,nl2); } }
}
int main(int argc,char**argv){
    N=argc>1?atoi(argv[1]):6;
    int d=argc>2?atoi(argv[2]):13;
    build(); tt=calloc(TTSIZE,sizeof(TT));
    BB z; bclr(&z);
    int v=solve(z,z,-1,1,d);
    printf("n=%d depth=%d root value %+d\n",N,d,v);
    if(v!=1){ printf("no first-player win at this depth\n"); return 1; }
    char line[512]; line[0]=0;
    walk(z,z,d,1,line);
    printf("n=%d strategy verified: %lld first-player decision nodes, %lld winning leaves\n",N,snodes,leaves);
    return 0;
}
